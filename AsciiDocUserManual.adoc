...
※入力したメールアドレスで既にアカウントを持っている場合は、パスワードの入力のみ求められます +
+
...
. 確認メールが届くので、メール内のURLをChromeで開いて、先ほど作成したアカウント情報を入力 +
+
image::Sourcetree07.png[width="200",align="left"]

. アカウントの認証に成功すると登録完了画面に遷移するので、 `次へ` をクリック +
+
image::Sourcetree08.png[width="300",align="left"]

. ツールのインストール画面に遷移するので `Git` にだけチェックを入れて、 `次へ` をクリック +
※既にGitをインストール済みの場合はチェックボックスは表示されません +
+
image::Sourcetree09.png[width="300",align="left"]

.. 社内のProxy環境化では、下記のエラーが表示されるので `キャンセル` をクリックし、手順11の画面も一旦閉じる +
+
image::Sourcetree09-1.png[width="300",align="left"]

.. Windowsのスタートメニューから `Sourcetree` を検索して起動すると、下記画面が表示されるので、一番下の選択肢の `Gitを使いたくない` をクリック +
+
image::Sourcetree09-2.png[width="300",align="left"]

.. Sourcetreeが起動したら、[ツール]→[オプション]から `ネットワーク` タブを選択し、`カスタムプロキシ設定を使用` にチェックを入れてProxyのアドレスとポート番号を入力し、`Git/Mercurialにプロキシ・・・` にチェックを入れる +
+
image::Sourcetree09-3.png[width="500",align="left"]
+
[NOTE]
====
...
+
image::Sourcetree09-4.png[width="500",align="left"]

.. 再び、手順bの画面が表示されるので、一番上の選択肢の `システム全体でなく、・・・` をクリック

.. Gitのダウンロードが始まるので、完了したら `OK` をクリックし、起動中のSourcetreeを一旦閉じる
...
. Sourcetreeが起動したのが確認できたら、起動中のSourcetreeを一旦閉じる +
+
image::Sourcetree11.png[width="300",align="left"]

. コマンドプロンプトの画面内にて、Atlassianアカウントを作成完了したか聞かれるので、`y` を入力して kbd:[Enter] を押す +
`y` の場合 ： コマンドプロンプトが表示されて処理が進むので自動的に閉じたら完了（5分程度かかります） +
`n` の場合 ： 再度、kbd:[Enter] を押してコマンドプロンプトを閉じる（別途、管理者へ相談してください） +
...
ここでは、テストサンプルのプレビューやPDFへの変換を行い、正しく環境構築ができたことを確認します。 +
...
===== [black]#プレビューを行う#
* ショートカット kbd:[Ctrl+K] → kbd:[V] で画面右側にプレビューが表示（以下はテストサンプルの参考プレビュー）
+
...
* UML図の作成時は、VScode拡張機能のPlantUMLを使用してプレビューした方が圧倒的に反応が早いので、参考までに実施方法を説明しておきます（参考情報なのでひとまずココは飛ばしても良いです）
