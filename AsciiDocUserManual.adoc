...
※社内のメールアドレスでOK
+
image::Sourcetree04.png[width="180",align="left"]

. 任意のユーザー名とパスワードを入力して、 `Sign up` をクリックで、アカウントを作成 +
※入力したメールアドレスで既にアカウントを持っている場合は、パスワードの入力のみ求められます +
+
image::Sourcetree05.png[width="180",align="left"]

. reCAPTCHAの画像認証の指示に従って選択を行い、 `確認` をクリック +
+
image::Sourcetree06.png[width="180",align="left"]

. 確認メールが届くので、URLを開いてアカウント情報を認証する +
+
image::Sourcetree07.png[width="180",align="left"]

. アカウントの認証に成功すると登録完了画面に遷移するので、 `次へ` をクリック +
+
image::Sourcetree08.png[width="300",align="left"]

. ツールのインストール画面に遷移するので、 `スキップ` をクリック +
+
image::Sourcetree09.png[width="300",align="left"]
//. `Preferences` 画面が表示されたら、チェックを外した状態のままで `次へ` をクリック +
...
. Sourcetreeが起動 +
+
image::Sourcetree11.png[width="600",align="left"]

. [ツール]→[オプション]を開く

.. （社内のProxy環境化の場合）`ネットワーク` タブを選択し、`カスタムプロキシ設定を使用` にチェックを入れてProxyのアドレスとポート番号の入力と、`Git/Mercurialにプロキシ・・・` にチェックを入れる +
+
...
+
image::Sourcetree13.png[width="550",align="left"]

.. 下記画面が表示されたら、一番上の選択肢の `システム全体でなく、・・・` をクリック
+
image::Sourcetree14.png[width="300",align="left"]

.. Gitのダウンロードが始まるので、完了したら `OK` をクリック
...
※最後にProxyの設定を元に戻すのは、PlantUMLで爆速プレビューを行うためのおまじない
====
...
※この手順は参考情報なので読み飛ばしてもOK（UML編集時に直面する問題に対する対応）

問題点： +

* AsciiDocプラグインにてUML図をプレビューすると、編集してプレビューする度に画像ファイルが別名で生成される
