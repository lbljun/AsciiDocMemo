...
├ template/                     // 文章のテンプレート一式
   ...
   └ test.adoc                       // AsciiDocテストサンプル
   └ test2.adoc                      // AsciiDocテストサンプル2
├ tool/                    　　 // インストール用バッチファイル
...
. コマンドプロンプトの画面内にて、環境がWindows10かどうか聞かれるので、Windouws10であれば `y` 、Windows7であれば `n` を入力して kbd:[Enter] を押す +
`y` の場合 ： 次の手順へ +
`n` の場合 ： コマンドプロンプトの画面内に書いてある手順に従ってください +
...
.. [ツール]→[オプション]から `ネットワーク` タブを選択し、Proxyの設定を行う +
`カスタムプロキシ設定を使用` を選択し、サーバー欄にはhttp://以降のアドレスを入力し、ポート番号も入力して、`Git/Mercurialにプロキシ・・・` にチェックを入れる +
+
...
.. ダウンロードが始まるので、完了したら `OK` をクリックし、その後、Sourcetree自体も一旦閉じる

.. 再度、Windowsのスタートメニューから `Sourcetree` を検索して起動すると、手順10の画面に戻ります +
※以降は気にせず基本的に全て `次へ` をクリックで大丈夫です
...
. ショートカット kbd:[Ctrl+,] で設定を開き、`proxy` を検索して、以下の設定を行う
+
----
Proxy : http://アドレス:ポート（Proxyの確認方法は先述通り）
Proxy Strict SSL : false（チェックを外す）
----
+
...
ショートカット kbd:[Ctrl+,] で設定を開き、`asciidoctor` を検索し、以下の設定を行います。
----
...
ショートカット kbd:[Ctrl+,] で設定を開き、`plantuml` を検索し、以下の設定を行います。
+
----
